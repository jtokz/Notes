...
There are other algorithms like Breadth-First Search (BFS) which behaves very similary to depth search but one difference, instead of always exploring the deepest node in the search tree, breadth-first search is always going to explore the shallowest node in the frontier, so the most recent node added to the frontier is the one we will explore next, so this one uses queue data estructure, queue is first-in, first-out data type. Like this:

![[Pasted image 20250624125057.png]]

This is the algorithm for StackFrontier(DSF) and how it takes, removes and explores a node

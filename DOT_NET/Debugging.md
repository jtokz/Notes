---
tags: [lang/csharp, area/basics, area/tools, type/reference]
---
...
```csharp
var friends = new List<string> { "Frank", "Joe", "Michelle", "Andy", "Maria", "ye", "Carlos", "Angelina" };
...
```csharp
// Logic to figure out the shortest friend names to invite to the party
...
```csharp
// ⚠️ BUG: modifica la lista original porque se pasa por referencia
public static List<string> GetPartyFriends(List<string> list, int count)
{
    ...
    while (partyFriends.Count < count)
    {
        ...
        list.Remove(currentFriend); // elimina de la lista original
    }
    return partyFriends;
}
```
...
```csharp
// ✅ FIX: usar una copia (buffer) para no modificar la lista original
public static List<string> GetPartyFriends(List<string> list, int count)
{
